...
return
     ...
     if (starts-with($path,'/media/'))   then concat("/controller/get-db-file.xqy?uri=", $path)
...
else if ($path eq "/admin/create.xqy")   then let $new-doc-url := concat($params[@name eq '~uri_prefix'],
                                         ...
                                         if (not(doc-available($new-doc-url))) then concat($orig-url,
                                                                                          "?~new_doc_url=",
                                                                                          $new-doc-url)
                                         else concat("/admin/transform.xqy?src=/admin", $params[@name eq '~edit_form_url'], "&amp;~orig_path=",
                                                                                        $params[@name eq '~edit_form_url'], "&amp;~doc_already_exists=yes")
else if (doc-available($doc-url))        then concat("/admin/transform.xqy?src=/admin", $path, "&amp;", $query-string, "&amp;~orig_path=", $path)
                                         else $orig-url
