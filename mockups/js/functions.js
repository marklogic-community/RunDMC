if(typeof jQuery != 'undefined') {
	$(function() {
      ...
      // end add media overlay
		...
		var addLink = '<a class="add_remove add_link">+&nbsp;Add link</a>';
		$('input[name=add_link]').replaceWith(addLink);
		...
		$('a.add_link').click(function() {
			...
			//$("fieldset.link div.addLink").remove();
			$(this).parent().siblings("fieldset.link").last().after('<fieldset class="link"><div><label>Link</label><input name="link[]" type="text" />' + removeLink + '</div><div><label>Version needed</label><input type="text" name="ce_version[]" /></div><div><label>Date</label><input type="text" name="ce_date[]" /></div></fieldset>');
...
// Create Array.indexOf function for browsers that don't have it
