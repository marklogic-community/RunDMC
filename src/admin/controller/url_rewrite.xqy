  ...
  (: create.xqy is special; we may need to send an error message without doing an external redirect
    ...
    return
      ...
      if ($slug-provided and not(doc-available($new-doc-url))) then concat($orig-url, "?~new_doc_url=", $new-doc-url)
        ...
        let $error-code := if (not($slug-provided)) then "no-slug" else "doc-exists"
        return
          ...
          concat("/admin/controller/transform.xqy?src=/admin",
            $params[@name eq '~edit_form_url'],
            "&amp;~orig_path=",
            $params[@name eq '~edit_form_url'],
            "&amp;~error_code=", $error-code)
     (: If it's an admin page, then render it :)
  else if (doc-available($doc-url)) then
    concat("/admin/controller/transform.xqy?src=/admin", $path, "&amp;", $query-string, "&amp;~orig_path=", $path)
