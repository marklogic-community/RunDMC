<xsl:stylesheet version="2.0"
  ...
  <!-- This determines where each REST doc gets stored (at what document URI) 
       -->
  <xsl:template mode="result-path-href" 
	  match="apidoc:function[@lib = $REST-libs]">
  ...
  <xsl:function name="api:REST-fullname">
    <xsl:param name="el"/>
    <xsl:sequence select="concat(api:translate-REST-resource-name($el/@name), 
	    ' (', ($el/@http-verb,'GET')[1], ')')"/>
  </xsl:function>
		    ...
		    <xsl:sequence select="substring-before( substring-after( 
			    $fullname,' (' ), ')')"/>
                  ...
                  <xsl:function name="api:verb-sort-key-from-REST-fullname">
                    <xsl:param name="fullname"/>
		    <xsl:variable name="verb-list" 
			    select="'GET','POST','PUT','HEAD','DELETE'"/>
		    <xsl:variable name="verb" 
			    select="api:verb-from-REST-fullname($fullname)"/>
  ...
  <xsl:function name="api:translate-REST-resource-name">
    ...
    <xsl:variable name="step2"
                  select="replace($step1,
                                       '\( ([^|)]+)  \|  \{ ([^}]+) \}  \)',
                                          &quot;['$1'-or-$2]&quot;, 'x')"/>         <!-- (default|{name}) => ['default'-or-name] -->

    <!-- Step 3: handle the braced alternatives -->
    <xsl:variable name="step3"
                  select="replace($step2,
                                  '\{ ([^|}]+)  \|  ([^}]+) \}',
                                           '[$1-or-$2]', 'x')"/>                    <!--        {id|name} => [id-or-name]        -->

    <!-- Step 4: replace remaining brackets with square brackets -->
    <xsl:variable name="step4" select="translate($step3, '{}()',
	    '[][]')"/>                      
    <!--           {name} => [name]              -->
    <!--           (name) => [name]              -->

    <!-- Step 4: replace ? with @ -->
    <xsl:value-of select="replace($step4, '\?', '@')"/>
  </xsl:function>
  ...
  <xsl:function name="api:reverse-translate-REST-resource-name">
    ...
    <!-- ASSUMPTION: Same assumption as above. Only these fixed patterns 
	 are supported. -->
    ...
    <xsl:variable name="step3" select="translate($step2, '[]',
	    '{}')"/>                        
    <!--   [name] => {name}           -->

    <!-- Step 4: replace @ with ? -->
    <xsl:value-of select="replace($step3, '@', '?')"/>
  </xsl:function>
