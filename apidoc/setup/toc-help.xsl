<xsl:stylesheet version="2.0"
  ...
  <xdmp:import-module namespace="http://marklogic.com/xdmp/admin/admin-forms" href="/MarkLogic/admin/admin-forms.xqy"/>
  ...
  <xsl:variable name="help-root-href" select="'/admin-help'"/>
  ...
  <xsl:template mode="help-toc" match="/">
    <!-- For error-checking only; helps propagate a more useful (directory not found) error -->
  ...
  <xsl:template mode="help-toc" match="*[number(@added-in) gt number($api:version)]"/>

  <xsl:template mode="help-toc" match="*">
    <xsl:variable name="element-decl" select="help:element-decl(.)"/>
    <xsl:variable name="exclusion-list" select="tokenize(normalize-space(@exclude),' '),
                                                if (@starting-with) then help:not-prefixed-names(.)
                                           else if (@auto-exclude)  then help:auto-exclude(.)
                                                                    else ()"/>
<!--
<xsl:message>$exclusion-list for <xsl:copy><xsl:copy-of select="@*"/></xsl:copy>: <xsl:copy-of select="$exclusion-list"/></xsl:message>
-->
    <xsl:variable name="line-after-list" select="tokenize(normalize-space(@line-after),' ')"/>
    <xsl:variable name="help-content">
      <xsl:copy-of select="af:displayHelp(root($element-decl)/*,                            (: $schemaroot    :)
                                          local-name(.),                                    (: $name          :)
      ...
      <xsl:if test="@append">
        <xsl:variable name="proxy-element" as="element()">
          <xsl:element name="{@append}" namespace="{namespace-uri-from-QName(resolve-QName(@append,.))}"/>
        </xsl:variable>
        <xsl:variable name="schema-element" select="root(help:element-decl($proxy-element))/*"/>
        <xsl:copy-of select="af:displayHelp($schema-element,
    ...
    <xsl:variable name="help-content-except-title">
      ...
      <xsl:copy-of select="if ($help-content/*:hr[2])
                          then $help-content/*:hr[2]/following-sibling::node()
                          else $help-content/*:span[1]/following-sibling::node()"/> <!-- one page (flexrep-domain) is missing the second <hr> -->
    </xsl:variable>
    <xsl:variable name="help-content-converted">
      <xsl:apply-templates mode="convert-help-content" select="$help-content-except-title"/>
    </xsl:variable>
    <xsl:variable name="title" select="if (@content-title)
                                      then @content-title
                                      else help:extract-title($help-content)"/>
    <node href="{help:path(.)}" display="{@display}" admin-help-page="yes">
      <xsl:apply-templates mode="help-node-open-att" select="."/>
      <title>
        <xsl:value-of select="$title"/>
      </title>
      <content>
        <xsl:copy-of select="if (@show-only-the-list) then ($help-content-converted//*:ul)[1]
      ...
      <xsl:apply-templates mode="#current"/>
          ...
          <!-- top two Help TOC levels pre-expanded -->
          <xsl:template mode="help-node-open-att" match="/* | /*/*">
  ...
  <xsl:function name="help:element-decl">
    ...
    <xsl:sequence select="$xsd-docs//xs:element[@name][QName(/*/@targetNamespace,@name) eq node-name($e)]"/>
  </xsl:function>

  <xsl:function name="help:extract-title">
    <xsl:param name="content"/>
    <!-- sometimes the source uses XHTML, sometimes not (e.g., x509.xsd) -->
    <xsl:sequence select="($content//*:span[@class eq 'help-text'])[1]/normalize-space(.)"/>
  </xsl:function>
  ...
  <xsl:function name="help:path">
    ...
    <xsl:sequence select="concat($help-root-href,
  ...
  <xsl:function name="help:resolve-repeat">
    ...
    <xsl:sequence select="root($e)//*[@id eq $e/@idref or node-name(.) eq resolve-QName($e/@name,$e)]"/>
  </xsl:function>
  ...
  <xsl:function name="help:auto-exclude">
    ...
    <xsl:for-each select="root($e)//*[namespace-uri(.) eq namespace-uri($e)]
                                     [not(. is $e)]
                                     [not(@added-in) or number(@added-in) le number($api:version)]">
      <xsl:variable name="this-name" select="local-name(.)"/>
      <!-- Automatically exclude this name, its plural forms, and whatever prefixed names it might stand for -->
  ...
  <xsl:function name="help:prefixed-names">
    <xsl:param name="e" as="element()"/>
    <xsl:sequence select="if ($e/@starting-with) then help:option-names($e)[starts-with(.,$e/@starting-with)]
                                                 else ()"/>
  </xsl:function>
  ...
  <xsl:function name="help:not-prefixed-names">
    <xsl:param name="e" as="element()"/>
    <xsl:sequence select="help:option-names($e)[not(starts-with(.,$e/@starting-with))]"/>
  </xsl:function>
  ...
  <xsl:function name="help:option-names">
    ...
    <xsl:variable name="complexType" select="root($decl)/*/xs:complexType[@name/resolve-QName(string(.),..) eq $decl/@type/resolve-QName(string(.),..)]"/>
    <xsl:sequence select="$complexType//xs:element/@ref/local-name-from-QName(resolve-QName(string(.),..))"/>
  </xsl:function>
